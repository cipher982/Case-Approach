...
# Main model function
def trainLGBM(df, targetFeature, clip_outliers=False):
    ...
    return score, y_pred, gbm, keep_cols, training_time
...
def train_predict():
    ...
    score, y_pred, gbm, keep_cols, training_time = trainLGBM(df, 'SalesRevenue', clip_outliers=False)
    ...
    for ID in storeIDs:
        ...
        criteria = pd.DataFrame({'Store_ID' : ID, # Set by loop
                                 'Fiscal_Qtr' : 3,
                                 'Fiscal_dayofWk' : 6,
                                 'Hour' : 12, 
                                 'AvgHourlyTemp' : 86,
    ...
    print("Total training time: {0}".format(training_time))
